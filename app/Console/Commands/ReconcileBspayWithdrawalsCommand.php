...
class ReconcileBspayWithdrawalsCommand extends Command
{
    protected $signature = 'withdrawals:reconcile-bspay
    ...
    protected $description = 'Consulta na BSPay saques PIX automáticos ainda pendentes e marca como pagos quando a API já liquidou.';
    ...
    public function handle(): int
    {
        ...
        if ($cred === null || ! $cred->is_connected) {
            $this->warn('Credencial BSPay (plataforma) não conectada.');
        ...
        if ($onlyId !== null && $onlyId !== '') {
            ...
            if (! in_array($w->status, ['pending', 'processing'], true) || $w->payout_provider !== 'bspay') {
                $this->warn('Saque ignorado (não está pending/bspay).');

                return self::SUCCESS;
            }
            $tx = trim((string) $w->payout_external_id);
            if ($tx === '') {
                $this->error('Saque sem payout_external_id; não é possível consultar na BSPay.');
